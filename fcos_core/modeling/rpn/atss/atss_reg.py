...
from .loss_reg import make_atss_loss_evaluator
...
class ATSSModule(torch.nn.Module):
    ...
    def forward(self, images, cls_features, reg_features, targets=None):
        ...
        if self.training:
            return self._forward_train(box_regression, centerness, targets, anchors)
        else:
            return self._forward_test(box_regression, centerness, anchors)

    def _forward_train(self, box_regression, centerness, targets, anchors):
        loss_box_reg, loss_centerness = self.loss_evaluator(
            box_regression, centerness, targets, anchors
        )
        losses = {
...
def build_atss_reg(cfg, in_channels):
