...
def im_detect_bbox_aug(arguements, model, images, device):
    ...
    boxlists_i = im_detect_bbox(arguements,
    ...
    if cfg.TEST.BBOX_AUG.H_FLIP:
        boxlists_hf = im_detect_bbox_hflip(arguements,
            model, images, cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST, device
        )
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        boxlists_scl = im_detect_bbox_scale(arguements, 
            model, images, scale, max_size, device
        )
        add_preds_t(boxlists_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            boxlists_scl_hf = im_detect_bbox_scale(arguements,
...
def im_detect_bbox(arguements, model, images, target_scale, target_max_size, device):
    """
    ...
    return model(arguements, images.to(device))


def im_detect_bbox_hflip(arguements, model, images, target_scale, target_max_size, device):
    """
    ...
    images = [transform(image) for image in images]
    images = to_image_list(images, cfg.DATALOADER.SIZE_DIVISIBILITY)
    boxlists = model(arguements, images.to(device))
...
def im_detect_bbox_scale(arguements, model, images, target_scale, target_max_size, device, hflip=False):
    """
    Computes bbox detections at the given scale.
    Returns predictions in the scaled image space.
    """
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(arguements, model, images, target_scale, target_max_size, device)
    else:
        boxlists_scl = im_detect_bbox(arguements, model, images, target_scale, target_max_size, device)
    return boxlists_scl
